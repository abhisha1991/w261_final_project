...
# # MASE Airlines: Delayed Flight Predictions
...
# 
# ##### References:
# Huo, Jiage & Keung, K.L. & Lee, C. & Ng, Kam K.H. & Li, K.C.. The Prediction of Flight Delay: Big Data-driven Machine Learning Approach. (2021). 
...
# 
# To start, we'll set up our environment by load all of the packages and variables that we'll need for reading and storing the data:

# In[ ]:


#imports
from pyspark.sql.functions import col, substring, split, when, lit, max as pyspark_max, countDistinct, count, mean,sum as pyspark_sum, expr, unix_timestamp, to_utc_timestamp, to_timestamp, concat, length, row_number
...
# ### 2.1 Core Datasets and Cleaning
...
# #### Airline Flight Data
# First, we will take a look at the raw provided airlines data:
...
# 
# The airlines dataset is robust, however, some features are unnecessary for our purpose and many rows are duplicated. An overview of the cleaning steps for the airline table are as follows:
# 
# 1. Feature removal
...
# 
# After cleaning, the airlines data is structured as follows:
...
# Next, we explored the weather station table to better understand the available features. The first 10 records are printed to show the original weather station data format:
...
# Next, we'll take a peek at our NOAA [weather dataset](https://www.ncdc.noaa.gov/orders/qclcd/). The first 10 rows of the raw dataset are shown to demonstrate the structure of the data:
...
#   - Dropped additional optional measurements related to precipitation, snow, dew point, and weather occurrences that are too sparsely recorded.
# 3. Replacing missing values
#   - As listed above, when recorded measurements are missing they are filled with various codes (9, 9999, +9999, etc)
#   - To avoid misinterpreting these as actual measurements, we replace these will nulls in our cleaning step
# 
# Below, we demonstrate the cleaned and unpacked dataset
...
# To better understand the benefit of dropping irrelevant weather records at this stage, let's take a look at how many weather measurements come from airports in our flights dataset:
...
# By subsetting to only relevant weather measurement locations, we werer able to drop 96.2% of the weather data. Without doing so, joining the datasets would be an enormously computationally expensive task.

# ### 2.2 Joining the Datasets
# 
# We joined the provided airline data with the origin and destination ICAO codes in the cleaning function. Now, we join each of the individual core datasets together as outlined in the diagram below. In order to effectively join our weather and flight datasets, we had to consider our goal: predicting flight delays *2 hours in advance*. The station data is joined to the cleaned airline data, then the weather at the origin airport is joined *at or before the prediction time*.

# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/airlines1.PNG?raw=true" />

# #### Stations to Airlines
# Due to the ICAO codes joined to the airlines from the OpenAirlines data sources, this join is straightforward. The stations data may be joined by this code (`neighbor_call` to `ICAO`). This is done twice: once for the origin stations and once for the destination stations. Below is the join function, run in the separate EDA notebook for both the subset dataset and the full datasets.
...
def join_weather_to_airlines(airlines, weather, prefix):
  # subset and rename
  weather = weather.dropDuplicates(subset=['STATION', 'DATE'])                   .select([col(c).alias(prefix+str.lower(c)) for c in weather.columns if c not in ['LATITUDE', 'LONGITUDE', 'NAME']])  
...
# 
# We recorded the time each join took to run, though were unable to note how many nodes the computation had access to due to fluctuations in availability during this time period.
...
# 
# The joined dataset is structured as follows:
...
# ### 2.3 Exploration and Visualizations
# Now that we have a fully joined dataset, we plot some visualizations of the existing features in order to better understand the data and develop hypotheses for our predictive models. First, we'll take a look at the percentage of flights delayed by month of the year.
...
monthly_agg = full.select(['dest_ICAO', 'year', 'month', 'dep_is_delayed', 'delay_minutes' ]).dropna(how='any').groupBy('year', 'month')        .agg(count('dest_ICAO').alias('n_flights'),             pyspark_sum(when(col('dep_is_delayed') == 'true', 1).otherwise(0)).alias('n_delayed'),             (pyspark_sum(when(col('dep_is_delayed') == 'true', 1).otherwise(0))/count('dep_is_delayed') * 100).alias('pct_origin_dow_delayed'),             mean('delay_minutes').alias('mean_origin_dow_delay'))        .withColumn("sort_order", col('month').cast(IntegerType()))        .orderBy(['year', 'sort_order'], ascending=1)
...
# This provides some context for the spread of the delayed flights. It would appear that the vast majority of flights have little to no delay from the planned departure time. Let's remove flights that are not delayed (<15 minutes), and flights that are delayed by more than 400 minutes.
...
# We see that the distributions are similar in that they both have long tails, with ATL having slightly higher kurtosis. Let's see if this same distribution shape is present on the larger dataset. We'll filter to the top 10 busiest airports in order to avoid clutter in the plot:

# In[ ]:


print("Top 10 airports by number of flights:")
top_10 = full.groupBy(['origin_ICAO']).agg(count('index_id').alias(f"n_flights"))        .orderBy('n_flights', ascending=0).limit(10).cache()
display(top_10)
...
filtered_full = full.filter(col('origin_ICAO').isin(top_10_ICAO))                    .filter(col("delay_minutes") >= 15)                    .filter(col("delay_minutes") < 400).toPandas()
...
full_weekly_agg = full.withColumn("hour_of_day", substring('planned_dep_time',1,2))        .groupBy('origin_ICAO', 'day_of_week', 'hour_of_day')\ # aggregate by weekday and hour
        ...
        .withColumn("sort_order", concat(col('hour_of_day'), col('day_of_week')).cast(IntegerType()))        .orderBy('sort_order', ascending=1)
...
# There do appear to be differences by weekday, though they do not align with our expectation that weekends would have more delays. We do think there is value in including this feature as well. Let's look at weekday and hour-of-day to see if more flights are delayed in the morning or afternoon.
...
# - Mean delay time (minutes)
# 
# This function is copied below, and again run in our EDA/Feature Engineering notebook.
...
def aggregate_keys(df, keys, name, count_only=False):
  if count_only:
    ...
    return df.groupBy(keys).agg(count('index_id').alias(f"n_flights_{name}"),                                count(when(col("dep_is_delayed"), col("dep_is_delayed"))).alias(f"n_delayed_{name}"),                                (count(when(col("dep_is_delayed"), col("dep_is_delayed")))/count('dep_is_delayed') * 100).alias(f"pct_delayed_{name}"),                                mean('delay_minutes').alias(f"mean_delay_{name}"))


# Let's take a look at some aggregates of our 3 month subset dataset. We'll aggregate by airport, route, and state to see if features about the flight routes or origin/destinations are substantially different:
...
# Let's take a look at our full dataset with the additional aggregate statistics joined on:
...
# ### 3.3 Holiday and Holiday-adjacent
...
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/holiday.PNG?raw=true" />

# ### 3.4 Mean imputation and Null Removal
# At this point, we have finished adding all features that we intend to add. In order to ensure the data is usable with all of our models we intend to employ, we will need to address missing values. There are two approaches to take, based on whether or not the feature is numeric or categorical:
# - **Categorical**: For nulls in categorical feature columns, we will replace the null with a string indicating it was missing (e.g. "NULL")
# - **Numeric**: For nulls in numeric feature columns, we will first generate an indicator variable that tracks which rows are being filled, then replace the nulls with the mean of the values that are not missing from that column (mean imputation)
# 
# The function below, copied from the EDA/Feature Engineering notebook, fills nulls as described, with a short example to illustrate:
...
# This concept is applied to the full dataset for features to be included in the models
...
# 
# In the image below, we can see that the 1st iteration shown in blue has training data date range between 1/1-6/30 and test data date range between 7/1-8/30. The second iteration shown in yellow trains between ranges 1/1-10/30 and testing is performed against 10/31-12/30. The third iteration shown in green trains between date ranges 1/1/2015-2/29/2016 and testing is done on range 3/1/2016-4/30/2016.  

# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/cvts.PNG?raw=true" width="800"/>

# We have written a custom function to do this for us. This can be found in the model notebook. We ensure that the training data is always larger than the test data. We choose to implement this in chunks of several months. In our official CV runs for the models, we chose the following splits of data.
# 
# For the first iteration, we select the following dates for train and test:
# 1. Train - (datetime.datetime(2015, 1, 1, 0, 0), datetime.datetime(2015, 8, 29, 0, 0))
# 2. Test - (datetime.datetime(2015, 8, 30, 0, 0), datetime.datetime(2015, 12, 28, 0, 0))
# 
# For the second iteration, we select the following dates for train and test:
# 1. Train - (datetime.datetime(2015, 1, 1, 0, 0), datetime.datetime(2016, 4, 27, 0, 0))
# 2. Test - (datetime.datetime(2016, 4, 28, 0, 0), datetime.datetime(2016, 8, 26, 0, 0))
# 
# We continue this pattern till we have exhausted all 4 years of data. We can think of the above as an expanding window strategy, where the training data starts from the beginning of the dataset in each iteration of the model and expands by a few months in every subsequent iteration. The test data begins where the training data date range ends and the test data ends at a fixed interval (4 months say) from its own starting point. The same can be visualized in the diagram above. 
# 
# Lastly, we had also implemented a sliding window strategy for train-test splits, such that - instead of starting at the beginning of the dataset in every iteration (2015, 1, 1, 0, 0) - we can slide our starting point by a fixed number of months in every iteration. We didn't choose to train with this strategy though, because we were concerned about the falling number of data points that resulted from the rebalancing that we discussed earlier (which may lead to lack of training data).
# 
# #### Iteration Based Learning
# 
# It is important to note that we split the data into the above iterations and then run our model (say logistic regression) from scratch for each of these iterations, thereby learning a new model for each iteration date range. Once we have all our models learned, we compute standard classification scores (precision, recall, accuracy, F1, AUROC, AUPR) for each model and we then aggregate these scores by finding mean, min, max, median values for each classification score across all our models (for logistic regression). This gives us a robust estimate on what the true classification performance looks like since we are getting scores across different time ranges, from multiple independent models. 
# 
# Finally, the above procedure described is repeated for our other classifiers - SVM, GBT, RF and their respective models also report aggregate classification statistics. This allows us to do an apples to apples comparison of these standard classification scores between different classifers.

# ## Section 5 - Algorithm Implementation

# #### Hypothesis Selection
# 
# After we finished feature engineering, we opted to subset the final features described in section 3.5. We formed several hypotheses based on these selected features, which we finally decided to train on. Some of these hypotheses were formed to test the effectiveness of the inclusion of certain collective variables that we believed would be important when predicting flight delays. For example, we know that bad weather is a huge determining factor when it comes to delayed flights, thus it is important to test the hypothesis of the inclusion of temperature and weather variables in our model as an independent hypothesis.
# 
# We ultimately ended up training 3 hypotheses:
# 
# ###### 1. Core Variables (Hypothesis 2 in Code)
# This hypothesis includes core variables that we include in our more advanced hypotheses as well. We believe that these are simple but valuable predictors to include in any model.
# 
# 1. **Numeric** - 'pct_delayed_from_origin', 'pct_delayed_to_dest', 'pct_delayed_for_route', 'pct_delayed_from_state', 'pct_delayed_to_state', 'flight_distance', 'origin_tmp_c'
# 2. **Categorical** - 'month', 'day_of_month', 'day_of_week', 'dep_hour', 'arr_hour', 'origin_ICAO', 'dest_ICAO', 'carrier', 'distance_group', 'holiday', 'poten_for_del'
# 
# ###### 2. Core + Geographic Environment Variables (Hypothesis 3 in Code)
# This hypothesis includes most variables in the Core Variables hypothesis, along with weather/altitude/pressure (environment) variables that are expected to impact flight delay. We performed this training because we were explicitly interested in the impact of these environment variables on flight delays.
# 
# 
# 1. **Numeric** - 'origin_altitude', 'origin_wnd_speed', 'origin_cig_cloud_agl', 'origin_vis_dist', 'origin_tmp_c', 'origin_dew_c', 'origin_slp_p','dest_altitude', 'planned_duration'
# 2. **Categorical** - 'month', 'day_of_month', 'day_of_week', 'dep_hour', 'arr_hour', 'origin_ICAO', 'dest_ICAO', 'carrier', 'distance_group', 'holiday', 'poten_for_del', 'canceled', 'origin_cig_cavok', 'origin_wnd_type', 'origin_vis_var', 'origin_city', 'dest_city'
# 
# There are a few subtle differences between this selection of features vs our core variables hypothesis. First, we notice that we have selected planned_duration instead of flight_distance. These 2 are highly correlated and thus we only select one of these to avoid multicollinearity in our models. We chose planned_duration in this hypothesis because we just wanted to give some variation in the variables we select across our hypotheses. Second, we notice that we have included both origin/destination city along with their origin/destination ICAO codes. This is to accommodate the fact that there may be cities with multiple airports. Notice that we have dropped the mean/percentage computed variables in this hypothesis as we're only interested in effects of environmental conditions on our scores.
# 
# ###### 3. Core + Computed Variables (Hypothesis 4 in Code)
# This hypothesis includes all core variables along with the features that were computed during the feature engineering phase. Notice that both percentage and mean variables are included in this model. Additionally, the indicator variables also contain weather related delays encoded in them. We believe this hypothesis is going to perform the best since it includes most of the meaningful variables spanning multiple areas such as weather, airport, carrier/late-aircraft, standard temporal (month, holiday, arrival hours, departure hours) and computed mean/percentage values. 
# 
# 1. **Numeric** - 'flight_distance', 'origin_tmp_c', 'pct_delayed_from_origin', 'mean_delay_from_origin', 'pct_delayed_to_dest', 'mean_delay_to_dest', 'pct_delayed_for_route', 'mean_delay_for_route', 'pct_delayed_from_state', 'mean_delay_from_state', 'pct_delayed_to_state', 'mean_delay_to_state'
# 2. **Categorical** - 'month', 'day_of_month', 'day_of_week', 'dep_hour', 'arr_hour', 'origin_ICAO', 'dest_ICAO', 'carrier', 'holiday', 'weather_window_del_ind', 'carrier_window_del_ind', 'security_window_del_ind', 'late_ac_window_del_ind', 'nas_window_del_ind', 'oa_avg_del_ind', 'da_avg_del_ind', 'carrier_avg_del_ind', 'poten_for_del', 'prev_fl_del'
# 
# If we do see better scores in this hypothesis compared to the others, we will continue to fine tune this as our ultimate selected hypothesis.

# #### "Core Variables" Hypothesis Results
# 
# We ran all 4 years of data using the iterative methodology described in section 4. Below we present the results of each of the models corresponding to this hypothesis. We see that all our models perform very well, however, logitstic regression seems to outperform all other models with this variable set. It is definitely encouraging to see good results on both precision and recall - 2 metrics that we were closely tracking. These are surfaced in the form of high F1 scores. All results below are running with "default" hyperparameters which can be found in the code notebook for each model.
# 
# It is important to read the results below in the form of mean, min, max and median values. For example, the first row for the logit image is corresponding to the mean scores across all iteration runs (across all years) for logistic regression with this "core variables" hypothesis. For the sake of explainability, say we trained logistic regression for 2 iterations - the first iteration (trained on 2015 data and tested against 4 months of 2016) gives a F1 score of 0.8 and the second iteration (trained on 2015+2016 data and tested against 4 months of 2017) gives a F1 score of 0.9. Then row 1 in the table below will be the mean of 0.8 and 0.9, which is 0.85. The second row will give us the min (0.8), the third row will give us the max (0.9) and the last row will give us the median (also 0.85)
# 
# We end up getting a maximum test F1 score of about 0.921 from our logit model. SVM is comes in second with a max F1 score of 0.845. The GBT model seemed to perform better than the RF model as expected. What was interesting about these results was that tree based models didn't do as well as we were expecting them to. This motivated us to try out different hypotheses and explore hyperparmeter tuning for tree based models to see if we could significantly improve F1 scores.
# 
# It is important to note that the test scores below are being reported on "unbalanced" test data. To perform effective learning on the train data, we had to rebalance the training data such that the positive and negative classes had equal number of examples. However, on the test data - we **do not** rebalance because it is representing a real world dataset where flights are expected to depart on time for the most part. To further our model run exploration, we do ultimately run a prediction against a "balanced" test dataset for our advanced hypotheses. This will be discussed in subsequent sections.

# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp2_default/hyp2_default_logit.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp2_default/hyp2_default_svm.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp2_default/hyp2_default_rf.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp2_default/hyp2_default_gbt.png?raw=true" width="800"/>
# 

# #### "Core + Geographic Environment Variables" Hypothesis Results

# Similar to our core model, this model also seemed to perform well on logistic regression, compared to the tree based models. Amongst the tree models, it seems that random forests got a good boost in their F1 scores and closed the gap that was previously present between RF and GBT models. Both classifiers now show maximum F1 scores of about 0.8, which is still worse than logistic regression. This shows that the RF model selected trees that were uncorrelated to each other and was able to learn effectively from multiple weak learners. What is also interesting is that the RF models seem to be performing slightly better than the GBT models (if we compare the mean scores of 0.744 vs 0.721). Note that all models were run with the "default" hyperparameters which can be found in the models notebook.
# 
# What was interesting about this set of results was that SVM reported a 0 F1 score against one of the iteration runs. This was unexpected and we suspect this is because of some error in the processing pipeline. We were unable to investigate the root cause as the same SVM pipeline seemed stable for other hypotheses. Further investigation is warranted to understand why this reported unusual results with default hyperparameter values. For now, we are reporting it. 
# 
# The results here also confirm that the weather and environment related variables do not give a very significant boost in classification scores all up, although they do help certain classifiers more than others. For example, between the previous hypothesis (core variables) and the current one (including environmental factors), Random Forests were able to gain significant boost in F1 scores (0.72 vs 0.80), whereas other algorithms didn't benefit much (SVM and logit whose F1 scores still hover around 0.83 and 0.92 respectively).

# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp3_default/hyp3_default_logit.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp3_default/hyp3_default_svm.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp3_default/hyp3_default_rf.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hyp3_default/hyp3_default_gbt.png?raw=true" width="800"/>

# #### "Core + Computed Variables" Hypothesis Results
# 
# This is the "best" model that the team chose since it had the highest scores for almost all models across the iterations. Note that all models were again run with default hyperparameters which can be found in the models notebook. In this hypothesis, almost all models benefited significantly. For example, SVM scores went up from 0.84 to 0.887 (max F1 scores compared to the previous hypothesis). Similarly, the tree based learners also benefited significantly. For example, GBT's F1 scores jumped from around 0.80 to around 0.90! Similarly, F1 scores for RF increased from 0.80 to 0.87. Logistic regression didn't benefit too much since it was already at around 0.92. This shows that once models have reached a high enough value in their classification metrics, any incremental gains are harder to come by!
# 
# The tree classifiers closed the gap and performed almost at par with logistic regression (0.90 max F1 score for GBT vs 0.92 max F1 score for Logistic Regression). This can be seen with their maximum F1 scores (row 3) against the test data. GBT performed slightly better than RF in this run - this is evidenced by slightly higher mean, min, max and median F1 scores betwen the 2 classifiers. Note that the test data on which prediction is being performed is unbalanced since it represents "real world" unbalanced data scenarios that we are predicting against.

# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_default/Hyp4_default_logreg.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_default/Hyp4_default_SVM.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_default/Hyp4_default_random_forest.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_default/Hyp4_default_GBT.png?raw=true" width="800"/>
# 
# 

# #### Hyper Parameter Tuning on Hypothesis 4 ("Core + Computed Variables")
# To further perform hyper parameter tuning on our selected hypothesis, we had 2 variants of hyper-parameters. For details on the hyperparameters used, please refer to the notebooks in the "ModelRuns" folder. At a high level:
# 1. **Custom1 hyper parameters** were trying to enforce stricter regularization (higher values for regParam, minInstancesPerNode, minInfoGain) 
# 2. **Custom2 hyper parameters** were trying to enforce more balanced values for regularization, while allowing higher flexibility for numTrees, maxIter, maxDepth. This was done to allow the trees to learn without constraints.
# 
# The results are presented below. We see that the scores do not change very much from what we had before. Again GBT seems to perform slightly better against RF as expected (0.87 vs 0.90 max F1 scores). Logistic regression still performs the best amongst these with a score of about 0.924. While more hyper parameter tuning is warranted for a full analysis on the effect of hyper parameters on model results, it does seem slightly intruiging that the existing set of tuning parameters didn't do much in changing the scores drastically. We may need to try more extreme values in our future model runs that may help move the needle on the results (for better or for worse!).

# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hy4_custom1/hyp4_custom1_logit.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hy4_custom1/hyp4_custom1_svm.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hy4_custom1/hyp4_custom1_rf.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/hy4_custom1/hyp4_custom1_gbt.png?raw=true" width="800"/>

# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_custom2/Hyp4_custom2_logreg.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_custom2/Hyp4_custom2_SVM.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_custom2/Hyp4_custom2_random_forest.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_custom2/Hyp4_custom2_GBT.png?raw=true" width="800"/>

# #### Rebalancing Test Data On Hypothesis 4 ("Core + Computed Variables")
# 
# As we had mentioned before, we had to perform rebalancing of the training data on our examples in order to provide similar ratio of examples for positive and negative classes. We did this against the train data for all the models we have seen so far. However, on the test data, we were not performing rebalancing since we don't expect to predict against a data set that is rebalanced. However, for the sake of completeness, we performed a model run for hypothesis 4 with default hyper parameter values with balanced test data as well. 
# 
# We see the results presented below. We observe that the F1 scores drop across the board but are still good for being used as classifiers. At this point, both GBT and Logistic Regression seem to give a maximum test F1 score of around 0.75. We will still choose to work with Logistic Regression at this time given that it is less complex and faster to train compared to GBT models.

# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_rebalanced/Hyp4_rebalanced_logreg.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_rebalanced/Hyp4_rebalanced_SVM.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_rebalanced/Hyp4_rebalanced_rf.png?raw=true" width="800"/>
# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/Screenshots/Hyp4_rebalanced/Hyp4_rebalanced_GBT.png?raw=true" width="800"/>
# 

# #### Trend Plot for F1 Scores for Classifiers
# 
# Below is a snippet of our F1 scores for train and test datasets for all the different hypotheses we employed in our training procedure. We notice that most of these F1 scores remain stable across all model runs - which shows that the classifiers were able to perform consistently well no matter what data (date ranges) they were training on.
# 
# We do note that there was one case where the SVM F1 score falls to 0, which has been described in section 5.

# 
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/f1-hyp2.png?raw=true" />
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/f1-hyp3.png?raw=true" />
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/f1-hyp4.png?raw=true" />
# <img src="https://github.com/abhisha1991/w261_final_project/blob/main/Team_07/Report/Images/f1-hyp4-rebal.png?raw=true" />
# 

# ## Section 6 - Conclusions

# In this project, we were able to successfully classify flight delays with high confidence using a metric of our choosing (F1 score). The motivation for choosing this metric has already been described above - keeping both the passengers and the airline as the main stakeholders in mind. We were able to achieve high scores across all our classifier models and we decided to choose the logistic regression model as our final model for a few reasons:
# 1. It consistently classified the data well across a range of hyper parameters
# 2. It was quicker to train against other models - a key consideration when we need to be updating the model on a real time basis with incoming flight data.
# 3. It is highly scalable and generalizes well
# 
# The only downside to logistic regression is hyperparameter tuning where we are required to explicitly choose thresholds and we are required to pre-process the data (null handling, normalization of features).
# 
# We performed limited hyperparameter tuning and trained a few different hypotheses across our models which resulted in the tree based models performing better and almost reaching at par with our logistic regression classifier. In our core variables hypothesis, tree based models were objectively worse than logistic regression. By the time we reached our more advanced hypothesis (Hypothesis 4), we were able to get the performance of tree learners to be at par with our selected classifier. More hyperparamter tuning is required for additional gains to our tree learners, but one can question whether the man hours required for this tuning is worth it for the incremental gains. If we anticipate sparse features in our feature space going forward, tree based models may not be the ideal choice anyway.
# 
# We also feature engineered some novel features that allowed us to predict with high classification metrics across the board. Some examples were holiday indicators, features around previous flight delay, features indicating delays 2-4 hours prior to departure and of course, the aggregate features for mean and percentage delay by different dimensions (routes, ICAO, state).
# 
# There are some improvements that can be made in the future to our current modeling state. One improvement that comes to mind is the the cross validation strategy for our train test splits. For example, we can utilize the rolling window feature we discussed in section 4 to train on a moving window of time. Additionally, we could implement block based time series cross validation to sample different "random blocks of time" in our dataset and split them into train-test. Some of these strategies may generalize well, allowing us to train effectively. From a feature selection standpoint - we can form additional hypotheses by combining other features that show up in the "feature importance" list of trees against newly engineered features. If we encounter large number of features that we want to consolidate while still capturing variation in the data, PCA can be employed to perform appropriate dimensionality reduction. We can also perform further hyper parameter tuning to improve scores, especially for tree based models - which we do eventually expect will outperform logistic regression. Lastly, we can try more advanced classification algorithms - including deep neural networks - that will help us achieve even higher scores (hopefully something closer to the SOTA - 96% classification - as discussed in the introduction).
# 
# Overall, we think our model performs sufficiently well in a variety of scenarios and we look forward to predicting delays accurately to improve customer experience and operational costs for MASE airlines!

# ## Section 7 - Application of Course Concepts
# 
# #### One Hot Encoding
# We applied various transformations to our data, one of which is one hot encoding (creates a binary column for each category and returns a sparse matrix). Some algorithms such as logistic regression and support vector machine, are unable to work with categorical data directly. Features that do not have a numeric order (or meaning) are converted to numerical form by first assigning each category to an integer value. Then they are one hot encoded to avoid algorithms treating the numbers as an attribute of significance. Each of the one hot encoded features has a value of 1 where the observations in the dataset has the categorical value, and 0 for all other observations. This is a good method for encoding categorical features because it does not assume an inherent order in the categories and is thus suited for both nominal and ordinal features.
# 
# Its important to note that one hot encoding should be applied to categorical columns with low to medium cardinality. However, if a column has very many unique values - a one hot encoded vector will result in very sparse vectors for that feature, which takes up both memory and increases compute time, while also possibly hurting algorithm training. If there are columns with extremely high cardinality, we should first group them into smaller buckets and then perform one hot encoding against these buckets. 
# 
# #### Gradient Descent and Importance of Normalization of Numeric Features
# 
...
# 
# #### Use of Columnar Storage (Parquet) for Faster Processing
# Since our datasets are very large and often require aggregation queries (group by, sum etc.), we decided to store our data in columnar format (parquet) for better performance. Parquet has a number of benefits:
# 1. Parquet automatically stores schema metadata at the end of the file for faster processing
# 2. It helps reduce storage requirements and provides higher IO throughput during ingestion and read - this was much needed by the team as they ingested multiple iterations of parquet files during the feature engineering phase.
# 3. It is faster when processing aggregate queries - which helped us greatly when creating our mean/percentage features
# 4. It allows for encoding nested data types and for efficiently storing sparsely populated data - we had a ton of sparsely populated columns in our dataset, so we relied on this feature heavily.
